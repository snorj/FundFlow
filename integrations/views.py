...
from .utils import encrypt_token, decrypt_token # Import encryption utils
from .services import verify_token # Import token verification service
...
class UpIntegrationSetupView(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        # --- Verify the token with Up Bank API *before* saving ---
        ...
        if not is_valid:
            ...
            return Response({"error": "Invalid or expired Up Personal Access Token."}, status=status.HTTP_400_BAD_REQUEST)

        # --- Token is valid, encrypt and save ---
        try:
            encrypted_token = encrypt_token(token)

            # Use update_or_create to handle both initial setup and updates
        ...
        except ValueError: # Catch encryption error from utils
             logger.error(f"[API Setup POST] Token encryption failed for user {user.id}.")
    ...
    def delete(self, request, *args, **kwargs):
        ...
        if deleted_count > 0:
            ...
            # Still return 204 as the desired state (no link) is achieved
            return Response(status=status.HTTP_204_NO_CONTENT)
...
class UpSyncTriggerView(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        # Check if the user has linked their Up account first
        try:
            ...
            return Response(
                {"error": "Up Bank account not linked. Please link your account in settings first."},
                status=status.HTTP_400_BAD_REQUEST
            )

        # Determine if it's the initial sync
        ...
        try:
            ...
            # Process the result from the sync logic
            if sync_result.get('success'):
                ...
                # Sync logic indicated failure, determine appropriate response status
                ...
                elif error_code == 'integration_not_found': # Should be caught above, but belt-and-suspenders
                     response_status = status.HTTP_400_BAD_REQUEST
                elif error_code in ['api_http_error', 'api_network_error']:
                     response_status = status.HTTP_503_SERVICE_UNAVAILABLE # Indicate upstream issue
                elif error_code == 'db_bulk_create_error':
